...
arbTxt = Caesar.Text <$> arbStr

instance Arbitrary Caesar.Text where
  arbitrary = arbTxt

prop_CaesarEncryptDecryptGeneratedTests :: Caesar.Text -> Int -> Bool
prop_CaesarEncryptDecryptGeneratedTests text key =
...
spec_encrypt = describe "Caesar.encrypt" $ do
  ...
  it "Plaintext: 'zzzz' Key: 1 should return Ciphertext: 'aaaa'" $ do
